...
init(autoreset=True)
import os
print(f"Created by ＣＹＢＥＲ ＡＤＨＩ ＯＦＣ 2025/06/30")
def is_valid_country_code(phone_number):
    pattern = r"^\+\d{1,4}\d{10,12}$"  # Modified to allow 10 to 12 digits
    return re.match(pattern, phone_number) is not None
while True:
    user_input = input(Fore.CYAN + "Enter the phone number with country code (e.g. +94.........): ")
    ...
    if is_valid_country_code(user_input):
        ...
        print(f"{Fore.RED}Please enter a valid number with a country code, like: +94123456789")
